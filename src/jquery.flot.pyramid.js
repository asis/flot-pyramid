...
var FlotPyramid = (function(){
  ...
  function processRawData(plot, series, datapoints) {
    ...
    // And also the X axis.
    //
    fixXaxis(options, series);
  }

  // Tells wether the given series should be drawn pointing towards
  // left, given the series data.
  function isLeftSeries(series) {
    return (series.pyramid.direction || 'R').match(/L|W/) != undefined;
  }
  ...
  function processDatapoints(plot, series, datapoints) {
    ...
    // Must stack the series?
    if (plot.getOptions().series.pyramid.stacked) {
      stackDatapoints(plot, series, swapped);
    }
  ...
  function stackDatapoints(plot, series, datapoints) {
    plot.pyramidLeftSeries = plot.pyramidLeftSeries || [];
    plot.pyramidRightSeries = plot.pyramidRightSeries || [];

    var isLeft = isLeftSeries(series),
        last = isLeft ? plot.pyramidLeftSeries : plot.pyramidRightSeries,
        i,
        l;
    ...
    if (last.length != 0) {
      ...
      for(i = 0; i < l; i += 3) {
        datapoints[i] += last[i];
        datapoints[i+2] = last[i];
      }
    }

    last = datapoints.slice(0);

    if (isLeft) {
      plot.pyramidLeftSeries = last;
    } else {
      plot.pyramidRightSeries = last;
    }
  }
  ...
  function fixXaxis(options, series) {
    var max,
        ...
        data = series.data,
        stacked = options.series.pyramid.stacked,
        isLeft = isLeftSeries(series),
        xaxis,
        values;
    ...
    // TODO: replace axis[0] with the relevant axis index
    xaxis = options.xaxes[0];

    // If the plot is stacked, stack the max too!
    if (stacked) {
      xaxis.pyramidLeft = xaxis.pyramidLeft || 0;
      xaxis.pyramidRight = xaxis.pyramidRight || 0;
      isLeft ? xaxis.pyramidLeft += max :
               xaxis.pyramidRight += max;

      xaxis.max = Math.max(xaxis.pyramidLeft, xaxis.pyramidRight);
    } else {
      // Compare the maximum value to the global maximum for the axis.
      xaxis.max = Math.max(max, currentMax);
    }
